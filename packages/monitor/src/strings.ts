export const strings = {
  ...
  queues: {
    ...
    phaseFailed: 'Failed',
    loadFailed: (reason: string) => `Couldn't load queue splits: ${reason}`,
    emptyTitle: 'No queue splits in this session',
    emptyBody:
      'Split a queue to receive only the messages that match your filter, while the rest keep flowing to the deployed app.',
    emptyHint: 'Configure queue splitting under feature.split_queues.',
    docsLabel: 'Queue splitting docs',
    docsUrl:
    ...
    podsLabel: 'pods',
    podPatched: (ok: boolean) => `patched ${ok ? '\u2713' : '\u2717'}`,
    podReady: (ok: boolean) => `ready ${ok ? '\u2713' : '\u2717'}`,
    metaSeparator: ' · ',
  },
} as const
