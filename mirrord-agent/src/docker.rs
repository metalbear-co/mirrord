...
impl DockerRuntime {
    pub fn new(container_id: String) -> Self {
        DockerRuntime { container_id, ns_path: None }
    }
}
...
impl Namespace for DockerRuntime {    
    async fn get_namespace(&mut self) -> Result<String> {
        ...
        let inspect_options = Some(InspectContainerOptions{
            size: false,
        });
        let inspect_response = client.inspect_container(&self.container_id, inspect_options).await?;
        let pid = inspect_response.state.unwrap().pid.unwrap();

        let ns_path = format!("/proc/{}/ns/net", pid).to_string();
        self.ns_path = Some(ns_path.clone());
        Ok(ns_path)
    }
}