...
unsafe extern "system" fn getaddrinfow_detour(
    ...
    let node_opt =
        Option::from(node_name).map(|ptr| unsafe { str_win::u16_buffer_to_string(ptr.as_wide()) });
    tracing::warn!("GetAddrInfoW_detour called for hostname: {:?}", node_opt);

    let service_opt = Option::from(service_name)
        .map(|ptr| unsafe { str_win::u16_buffer_to_string(ptr.as_wide()) });
