...
use winapi::{
    shared::{
        minwindef::INT,
        ws2def::{SOCKADDR, SOCKADDR_IN, AF_INET, AF_INET6},
...
pub unsafe fn socket_address_to_sockaddr(layer_addr: &SocketAddress) -> Result<(SOCKADDR, INT), String> {
...
/// security issues.
pub fn validate_buffer_params(_buffer: *mut u8, size: *mut u32, max_reasonable_size: usize) -> Option<usize> {
    if size.is_null() {
        return None;
    }
    ...
    K: Clone + std::hash::Hash + Eq,
{
    if cache.len() >= max_size {
        ...
        tracing::debug!("Cache evicted {} old entries, {} remaining", 
                       max_size / 2, cache.len());
    }
}
