...
fn error_success() {
    ...
    assert_eq!(
        error.get_formatted_error(),
        Some("The operation completed successfully.".into())
    );
}
...
fn error_file_not_found() {
    ...
    assert_eq!(
        error.get_formatted_error(),
        Some("The system cannot find the file specified.".into())
    );
}
