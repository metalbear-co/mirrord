...
mod main {
    ...
    mod tests {
        ...
        fn is_sip_true() {
            assert!(matches!(
                get_sip_status(Path::new("/bin/ls"), &[]),
                Ok(SipBinary(_))
            ));
        }
        ...
        fn is_sip_false() {
            ...
            f.flush().unwrap();
            assert!(matches!(get_sip_status(f.path(), &[]).unwrap(), NoSip));
        }

        #[test]
        fn is_sip_notfound() {
            let err = get_sip_status(Path::new("/donald/duck/was/a/duck/not/a/quack/a/duck"), &[])
                .unwrap_err();
        ...
        fn patch_binary_fat() {
            ...
            assert!(matches!(get_sip_status(&output, &[]).unwrap(), NoSip));
        ...
        fn patch_binary_fat_with_arm64() {
            ...
            assert!(matches!(
                get_sip_status(&patched_path_buf, &[]).unwrap(),
                SipStatus::NoSip
            ));
            // Check DYLD_* features work on it:
