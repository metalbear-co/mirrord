...
use http::Request;
use mirrord_config::{LayerConfig, config::ConfigContext, env_key};
use tracing::Level;
...
pub(crate) async fn subscribe_command(args: SubscribeArgs) -> CliResult<()> {
    let mut cfg_context = ConfigContext::default()
        ...
        .override_env_opt(env_key::MIRRORD_ENV_KEY, args.key.as_ref());
    ...
    // `--key` takes precedence, falling back to the `key` field in the config file. An
    // autogenerated key (when neither is set) is meaningless here, so require one explicitly.
    let key = layer_config
        .key
        .provided()
        .ok_or(CliError::SubscribeKeyRequired)?;
    ...
    while let Some(line) = lines.next().await {
        ...
        // Flush per event so it streams immediately through a pipe.
