...
fn extract_portforward_configs(config: &DatabaseBranchesConfig, key: &str) -> HashSet<Pf> {
    ...
    for branch in config.iter() {
        ...
        // than the shared `connection` block. It reuses the params path with no separate port
        // var:
        // `port: None` tells the resolver to split the host var's value, and the absent scheme
        // makes it write the branch address back as a bare `host:port`.
...
mod tests {
    ...
    use mirrord_config::feature::database_branches::{
        ...
        MysqlBranchConfig, ParamSource, TargetEnvironmentVariableSource,
    };
