...
impl GlobalConfig {
    ...
    async fn mutate<E>(
        update: impl FnOnce(&mut LayerFileConfig) -> Result<(), E> + Send + 'static,
    ) -> Result<LayerFileConfig, E>
    where
    ...
    pub(crate) fn validate_value(
        value: Value,
    ) -> Result<LayerFileConfig, GlobalConfigValidationError> {
        if value.is_object().not() {
            return Err(GlobalConfigValidationError::ExpectedObject);
        }

        let config: LayerFileConfig = serde_json::from_value(value)?;
        let mut context = ConfigContext::default().strict_env(true);
        config.clone().generate_config(&mut context)?;
        Ok(config)
    }
...
impl FieldPath {
    ...
    fn set_value(&self, document: &mut Value, new_value: Value) -> Result<(), GlobalConfigError> {
        let (last, parents) = self.segments.split_last().ok_or_else(|| {
            GlobalConfigError::invalid_field_path(
                &self.dotted,
                "field path segments cannot be empty",
            )
        })?;
        ...
        for segment in parents {
            let object = current.as_object_mut().ok_or_else(|| {
                GlobalConfigError::mutation_error(
                    &self.dotted,
                    "cannot traverse a non-object value",
                )
            })?;
            let next = object
                .entry(segment)
            ...
            if next.is_object().not() {
                return Err(GlobalConfigError::mutation_error(
                    &self.dotted,
                    "cannot traverse a non-object value",
                ));
            }
        ...
        let object = current.as_object_mut().ok_or_else(|| {
            GlobalConfigError::mutation_error(
                &self.dotted,
                "cannot set a field on a non-object value",
            )
        })?;
    ...
    fn unset_value(&self, document: &mut Value) -> Result<(), GlobalConfigError> {
        let (last, parents) = self.segments.split_last().ok_or_else(|| {
            GlobalConfigError::invalid_field_path(
                &self.dotted,
                "field path segments cannot be empty",
        ...
        for segment in parents {
            let object = current.as_object_mut().ok_or_else(|| {
                GlobalConfigError::mutation_error(&self.dotted, "value does not exist")
            })?;
    ...
    fn apply_set(
        self,
        config: &mut LayerFileConfig,
        value: Value,
    ) -> Result<(), GlobalConfigError> {
        let mut candidate = serde_json::to_value(&*config)?;
        self.set_value(&mut candidate, value)?;
        *config = GlobalConfig::validate_value(candidate)?;
        Ok(())
    }

    fn apply_unset(self, config: &mut LayerFileConfig) -> Result<(), GlobalConfigError> {
        let mut candidate = serde_json::to_value(&*config)?;
        self.unset_value(&mut candidate)?;
        *config = GlobalConfig::validate_value(candidate)?;
        Ok(())
    }
}
...
mod tests {
    ...
    fn file_config(value: Value) -> LayerFileConfig {
    ...
    fn apply_test_set(
        ...
        path: &str,
        raw_value: &str,
    ) -> Result<LayerFileConfig, GlobalConfigError> {
    ...
    fn set_parses_valid_json_into_typed_values() {
        let config = apply_test_set(&LayerFileConfig::default(), "operator", "true").unwrap();
    ...
    fn set_accepts_quoted_json_to_force_a_string() {
        let config =
            apply_test_set(&LayerFileConfig::default(), "agent.image", r#""true""#).unwrap();

        assert_eq!(
            serde_json::to_value(config).unwrap(),
            serde_json::json!({"agent": {"image": "true"}})
        );
    }

    #[test]
    fn dotted_path_segments_treat_slashes_and_tildes_literally() {
        let path = FieldPath::parse("agent/image.registry~name".to_owned()).unwrap();
        let error = apply_test_set(
            &LayerFileConfig::default(),
            "agent/image.registry~name",
            "custom.image/latest",
        )
        .unwrap_err();

        assert_eq!(path.segments, ["agent/image", "registry~name"]);
        assert!(error.to_string().contains("agent/image"));
    }
    ...
    fn set_rejects_unknown_regular_config_field() {
        let error = apply_test_set(&LayerFileConfig::default(), "booga", "true").unwrap_err();
    ...
    fn set_rejects_value_with_wrong_type() {
        let error = apply_test_set(&LayerFileConfig::default(), "operator", "wedel").unwrap_err();

        assert!(error.to_string().contains("boolean"));
    }

    #[test]
    fn unset_returns_dotted_errors_for_missing_values() {
        let missing_final = apply_test_unset(&LayerFileConfig::default(), "operator").unwrap_err();
        let missing_intermediate = apply_test_unset(
            &file_config(serde_json::json!({"operator": true})),
            "operator.value",
        )
        .unwrap_err();

        for error in [&missing_final, &missing_intermediate] {
            assert!(error.to_string().contains("value does not exist"));
        }
        assert!(missing_intermediate.to_string().contains("operator.value"));
    }

    #[test]
    fn unset_removes_nested_value_without_persisting_resolved_defaults() {
        let configured = apply_test_set(
            &LayerFileConfig::default(),
            "agent.image",
            "custom.image/latest",
        )
    ...
    fn mutation_errors_use_the_original_dotted_path() {
        let config = file_config(serde_json::json!({
            "feature": {"network": {"incoming": {"ports": []}}}
        }));
        let set_error =
            apply_test_set(&config, "feature.network.incoming.ports.named~port", "80").unwrap_err();
        let unset_error =
            apply_test_unset(&LayerFileConfig::default(), "feature/network.incoming~mode")
                .unwrap_err();
        ...
        assert_eq!(
            set_error.to_string(),
            "Cannot update global configuration at `feature.network.incoming.ports.named~port`: cannot traverse a non-object value"
        );
        assert!(
            unset_error
                .to_string()
    ...
    #[tokio::test]
    async fn set_incoming_mode_rejects_scalar_shorthand_and_preserves_bytes() {
        let directory = tempdir().unwrap();
        let path = directory.path().join("mirrord.json");
        let original = br#"{"feature":{"network":{"incoming":"mirror"}}}"#;
        ...
        assert_eq!(
            error.to_string(),
            "Cannot update global configuration at `feature.network.incoming.mode`: cannot traverse a non-object value"
        );
        assert_eq!(fs::read(path).await.unwrap(), original);
    }

    #[tokio::test]
    async fn set_agent_image_nested_path_rejects_valid_string_and_preserves_bytes() {
        let directory = tempdir().unwrap();
        let path = directory.path().join("mirrord.json");
        let original = br#"{"agent":{"image":"example.com/mirrord:latest"}}"#;
        fs::write(&path, original).await.unwrap();
        let field_path = FieldPath::parse("agent.image.repository".to_owned()).unwrap();
        let value = parse_value("custom".to_owned());
        ...
        assert_eq!(
            error.to_string(),
            "Cannot update global configuration at `agent.image.repository`: cannot traverse a non-object value"
        );
        assert_eq!(fs::read(path).await.unwrap(), original);
    }

    #[test]
    fn set_value_rejects_all_non_object_intermediate_values_without_mutating() {
        for parent in [
            ...
            let path = FieldPath::parse("parent.child".to_owned()).unwrap();

            let error = path
                .set_value(&mut document, Value::String("value".to_owned()))
                .unwrap_err();

            assert_eq!(
                error.to_string(),
                "Cannot update global configuration at `parent.child`: cannot traverse a non-object value"
            );
...
"#;
        ...
        for edit in [
            ...
            let result = match edit {
                RejectedEdit::InvalidPath => {
                    FieldPath::parse("agent..image".to_owned()).map(|_| LayerFileConfig::default())
                }
                RejectedEdit::UnknownField => {
