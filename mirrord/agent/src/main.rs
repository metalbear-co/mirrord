...
mod agent {
    ...
    use mirrord_protocol::{
        ClientMessage, DaemonCodec, DaemonMessage, GetEnvVarsRequest, LogMessage,
    };
    ...
    struct State {
        ...
        /// connected.
        should_pause: bool,
        /// This is an option because it is acceptable not to pass a container runtime and id if
        /// not pausing. When those args are not passed, container is None.
        container: Option<Container>,
    }

    /// This is to make sure we don't leave the target container paused if the agent hits an error
    /// and exits early without removing all of its clients.
    impl Drop for State {
        fn drop(&mut self) {
