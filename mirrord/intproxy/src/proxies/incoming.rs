...
//         ) { (Some(path), None, None) => {
//           Self::Filter(HttpFilter::Path(Filter::new(path.into()).unwrap())) } (None,
//           Some(header), None) => {
//           Self::Filter(HttpFilter::Header(Filter::new(header.into()).unwrap())) } (None, None,
//           Some(header)) => { Self::HeaderDeprecated(Filter::new(header.into()).unwrap()) } (None,
//           None, None) => Self::None, _ => { unreachable!("Only one HTTP filter can be specified
//           at a time, please report bug, config should fail before this.") }
//         }
//     }
// }
...
//         connection_id: ConnectionId,
//         layer_address: SocketAddr,
//     ) -> Self { let (data_tx, data_rx) = mpsc::channel(512);
...
//         agent_sender: AgentSender,
//         layer_sender: LayerSender,
//     ) -> Self { Self { mode: Mode::new(config), agent_sender, layer_sender, subscriptions_queue:
//       Default::default(), layer_listeners: Default::default(), connections: Default::default(), }
//     }
...
//     ) -> Result<()> { if let Some(previous) = self.layer_listeners.get_mut(&subscribe.port) { let
//       PortSubscribe { port, listening_on } = subscribe;
