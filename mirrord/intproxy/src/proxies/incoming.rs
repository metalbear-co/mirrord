...
/// Extracts method, path, and host from an [`InternalHttpRequest`] and emits
/// a [`MonitorEvent::IncomingRequest`].
fn emit_incoming_request_event<B>(monitor_tx: &MonitorTx, req: &InternalHttpRequest<B>) {
    ...
    let host = req
        ...
        .unwrap_or("")
        .to_owned();
    monitor_tx.emit(MonitorEvent::IncomingRequest { method, path, host });
}
    ...
    async fn handle_chunked_request(
        ...
        match request {
            ChunkedRequest::StartV1(request) => {
                emit_incoming_request_event(&self.monitor_tx, &request.internal_request);
            ...
            ChunkedRequest::StartV2(request) => {
                emit_incoming_request_event(&self.monitor_tx, &request.request);
        ...
        match message {
            ...
            DaemonTcp::Data(data) => {
                ...
                if self.http_sniff_pending.remove(&data.connection_id)
                    && let Some((method, path, host)) = try_parse_http_request(&data.bytes)
                {
                    self.monitor_tx
