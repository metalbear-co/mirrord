use mirrord_protocol::{
    file::{
        AccessFileRequest, CloseDirRequest, CloseFileRequest, FdOpenDirRequest, GetDEnts64Request,
        OpenFileRequest, OpenRelativeFileRequest, ReadDirRequest, ReadFileRequest,
        ReadLimitedFileRequest, SeekFileRequest, WriteFileRequest, WriteLimitedFileRequest,
        XstatFsRequest, XstatRequest,
    },
...
impl FileHandler {
    ...
    pub async fn handle_daemon_message(&mut self, message: FileResponse) -> Result<()> {
        let res = match message {
            ...
            FileResponse::WriteLimited(write) => self.write_limited_queue.resolve_one(write).await,
            FileResponse::Xstat(xstat) => self.xstat_queue.resolve_one(xstat).await,
            FileResponse::XstatFs(xstatfs) => self.xstatfs_queue.resolve_one(xstatfs).await,
            FileResponse::ReadDir(read_dir) => self.readdir_queue.resolve_one(read_dir).await,
            FileResponse::OpenDir(open_dir) => self.opendir_queue.resolve_one(open_dir).await,
            #[cfg(target_os = "linux")]
    ...
    pub async fn handle_hook_message(
        ...
        match request {
            FileOperation::Open(open) => self.handle_hook_open(open, request_id).await,
            FileOperation::OpenRelative(open_relative) => {
                self.handle_hook_open_relative(open_relative, request_id)
                    .await
            }
            FileOperation::Read(read) => self.handle_hook_read(read, request_id).await,
            FileOperation::ReadLimited(read) => {
                self.handle_hook_read_limited(read, request_id).await
            }
            ...
            FileOperation::WriteLimited(write) => {
                self.handle_hook_write_limited(write, request_id).await
            }
            FileOperation::Close(close) => self.handle_hook_close(close, request_id).await,
            FileOperation::Access(access) => self.handle_hook_access(access, request_id).await,
            FileOperation::Xstat(xstat) => self.handle_hook_xstat(xstat, request_id).await,
            FileOperation::XstatFs(xstatfs) => self.handle_hook_xstatfs(xstatfs, request_id).await,
            FileOperation::ReadDir(read_dir) => {
                self.handle_hook_read_dir(read_dir, request_id).await
            }
            FileOperation::FdOpenDir(open_dir) => {
                self.handle_hook_fdopen_dir(open_dir, request_id).await
            }
            FileOperation::CloseDir(close_dir) => {
                self.handle_hook_close_dir(close_dir, request_id).await
            }
            #[cfg(target_os = "linux")]
            FileOperation::GetDEnts64(getdents64) => {
    ...
    #[tracing::instrument(level = "trace", skip(self))]
    async fn handle_hook_read_limited(&mut self, read: ReadLimited, request_id: u64) -> Result<()> {
        let ReadLimited {
            remote_fd,
            buffer_size,
            start_from,
    ...
    #[tracing::instrument(level = "trace", skip(self))]
    async fn handle_hook_fdopen_dir(&mut self, open_dir: FdOpenDir, request_id: u64) -> Result<()> {
        let FdOpenDir { remote_fd } = open_dir;
