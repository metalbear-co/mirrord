    ...
    P: AsRef<str> + AsRef<OsStr>,
{
    ...
    let mut config = if let Some(kubeconfig) = kubeconfig
        ...
        .filter(|p| p.as_os_str().is_empty().not())
        .collect::<Vec<_>>()
        && paths.is_empty().not()
    {
        let parsed_kube_config = paths.iter().try_fold(Kubeconfig::default(), |m, p| {
            let binding = p.to_string_lossy();
