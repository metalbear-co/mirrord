//! Manual implementation of serialization and deserialization of the [`Workflow`].
//!
//! Some fields of the [`Workflow`] are actually associated fields of [`Resource`], so they're
//! simply not serialized if we just use `#[derive(Serialize, Deserialize)]`.
//!
...
use crate::api::kubernetes::workflow::Workflow;
...
impl Serialize for Workflow {
        ...
        S: serde::Serializer,
    {
        ...
        state.serialize_field("apiVersion", Workflow::API_VERSION)?;
        state.serialize_field("kind", Workflow::KIND)?;
                ...
                while let Some(key) = map.next_key()? {
                    match key {
                        Field::ApiVersion => {
                            ...
                            if api_version != Workflow::API_VERSION {
