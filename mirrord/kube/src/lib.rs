...
//                     .filter(|(key, _)| key != MIRRORD_GUARDED_ENVS && key !=
// EnvVarGuard::ENV_VAR)                     .collect();
...
//         Ok(target.clone())
//     } else {
//         Err(KubeApiError::InvalidTarget(
//             "No target specified. Please set the `MIRRORD_IMPERSONATED_TARGET` environment
// variable.".to_string(),         ))
//     }
// }
...
//         let client = Client::try_from(kube_config).map_err(KubeApiError::KubeError)?;
...
//                 &PostParams::default(),
//                 to_vec(&ephemeral_containers_subresource).map_err(KubeApiError::from)?,
//             )
//             .await
//             .map_err(KubeApiError::from)?;
...
//                                 ],
//                                 "command": agent_command_line,
//                                 "env": [{"name": "RUST_LOG", "value":
// self.config.agent.log_level}],                                 "resources": // Add requests to avoid getting defaulted https://github.com/metalbear-co/mirrord/issues/579
//                                 {
//                                     "requests":
//                                     {
//                                         "cpu": "10m",
...
//             _ => Err(KubeApiError::ContainerRuntimeParseError(
...
//             .next()
//             .ok_or_else(||
// KubeApiError::ContainerRuntimeParseError(container_id_full.to_string()))?
// .to_owned();
