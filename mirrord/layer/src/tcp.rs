...
//                     the application already closed the socket. The connection will not be
// handled."                 )
//             } else {
//                 info!(
//                     "Got incoming tcp connection (mirrord connection id: {connection_id}) after \
...
//     ) -> Result<(), LayerError> { match message { TcpIncoming::Listen(listen) =>
//       self.handle_listen(listen, tx).await, TcpIncoming::Close(port) =>
//       self.handle_server_side_socket_close(port, tx).await, }
//     }
...
//     ) -> Result<TcpStream, LayerError> { let remote_destination_port =
//       tcp_connection.destination_port; let local_destination_port = self .port_mapping_ref()
//       .get_by_right(&tcp_connection.destination_port) .map(|p| { trace!("mapping port {} to {p}",
//       &tcp_connection.destination_port); *p }) .unwrap_or(tcp_connection.destination_port);
