...
use std::ffi::c_void;
use std::{cmp::Ordering, fs::File, io::Read, panic, sync::Arc};
...
use mirrord_layer_core::{
    EXECUTABLE_ARGS, EXECUTABLE_PATH, HookManager, LayerInitialization, LoadType,
    PROXY_CONNECTION_TIMEOUT, layer_pre_initialization, load_only_layer_start, replace,
};
#[cfg(doc)]
...
use crate::socket::hooks::MANAGED_ADDRINFO;
...
///
/// Calls into `mirrord-layer-core` for the shared bootstrap and keeps the
/// feature-specific startup branches here.
#[ctor]
fn mirrord_layer_entry_point() {
    ...
    let res = panic::catch_unwind(|| match layer_pre_initialization() {
        ...
        Ok(LayerInitialization { config, load_type }) => {
            ...
            match load_type {
                LoadType::Full => mirrord_layer_core::layer_start(config, enable_hooks),
                #[cfg(target_os = "macos")]
                LoadType::SIPOnly => {
                    sip_only_layer_start(config, patch_binaries, skip_patch_binaries)
                }
