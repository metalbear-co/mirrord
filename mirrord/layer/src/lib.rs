...
// NOTE(gabriela): this is flagged out on Windows because we can't build layer,
// this is not an ideal solution! please return to this once there's an RFC on
// this, or once there's a resolution.
//
// the ideal scenario would be having conditional build targets at cargo.toml
// level so that, in crates like `layer-lib`, we can cfg-gate `mirrord-layer`
// and further cfg-gate interactions with `layer`.
#![cfg_attr(not(windows), deny(unused_crate_dependencies))]
