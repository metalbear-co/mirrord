...
//     loop {
//         select! {
//             // Read the application's response from the socket and discard the data, so that the
// socket doesn't fill up.             read = local_stream.read(&mut buffer) => {
//                 match read {
...
//                     },
//                     None => {
//                         // The remote stream has closed, sleep 1 second to let the local stream
// drain (i.e if a response is being sent)                         debug!("remote stream closed");
...
//     ) -> Result<()> { self.apply_port_mapping(&mut listen); let request_port =
...
//     ) -> std::result::Result<(), LayerError> { error!("Error: Mirror handler received http
//       request."); debug_assert!(false); Ok(())
//     }
...
//         port: Port,
//         tx: &Sender<ClientMessage>,
//     ) -> std::result::Result<(), LayerError> { if self.ports_mut().remove(&port) {
//       tx.send(ClientMessage::Tcp(LayerTcp::PortUnsubscribe(port))) .await .map_err(From::from) }
//       else { // was not listening on closed socket, could be an outgoing socket. Ok(()) }
//     }
// }
