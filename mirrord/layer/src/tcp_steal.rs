...
//     ) -> Result<(), LayerError> { let stream = self.create_local_stream(&tcp_connection).await?;
...
//         } else {
//             trace!(
//                 "mirrord got new stolen incoming tcp data for a connection that is already
// closed: \                 {:?}",
//                 data.connection_id,
//             );
//             return Ok(());
...
//     /// If this is the first filtered HTTP from its remote connection to arrive at this layer, a
// new     /// local connection will be started for it, otherwise it will be sent in the existing
// local     /// connection.
//     #[tracing::instrument(level = "trace", skip(self))]
//     async fn handle_http_request(
//         &mut self,
//         request: HttpRequestFallback,
//     ) -> Result<(), LayerError> { if let Some(sender) =
//       self.http_request_senders.get(&request.connection_id()) { trace!( "Got an HTTP request from
//       an existing connection, sending it to the client task \ to be forwarded to the
//       application." ); Ok(sender .send(request) .await .map_err::<HttpForwarderError,
//       _>(From::from)?) } else { Ok(self.create_http_connection(request).await?) }
//     }
...
//     ) -> Result<(), LayerError> { let original_port = listen.requested_port;
//       self.apply_port_mapping(&mut listen); let request_port = listen.requested_port;
...
//         port: Port,
//         tx: &Sender<ClientMessage>,
//     ) -> Result<(), LayerError> { if self.ports_mut().remove(&port) { // There is a known issue -
//       https://github.com/metalbear-co/mirrord/issues/1575 - where // the agent is currently not
//       able to keep ongoing connections, so once we send this // message, any ongoing connections
//       are going to be interrupted.
//       tx.send(ClientMessage::TcpSteal(LayerTcpSteal::PortUnsubscribe( port, ))) .await
//       .map_err(From::from) } else { // was not listening on closed socket, could be an outgoing
//       socket. Ok(()) }
//     }
// }
...
//         port_mapping: BiMap<u16, u16>,
//         http_filter_settings: HttpFilterSettings,
//     ) -> Self { Self { ports: Default::default(), write_streams: Default::default(),
//       read_streams: Default::default(), http_request_senders: Default::default(),
//       http_response_sender, http_filter_settings, port_mapping, }
//     }

//     /// Remove a layer<->local-app connection.
//     /// If the connection is still open, it will be closed, by dropping its read and write
// streams.     fn remove_connection(&mut self, connection_id: ConnectionId) {
...
//     /// Spawn a task that receives requests on a channel and sends them to the application on
// that     /// new TCP connection. The sender of that channel is stored in
// [`self.request_senders`].     /// The responses from all the http client tasks will arrive
// together at     /// [`self.response_receiver`].
//     #[tracing::instrument(level = "trace", skip(self))]
//     async fn create_http_connection(
//         &mut self,
//         http_request: HttpRequestFallback,
//     ) -> Result<(), LayerError> { let port = http_request.port(); let connection_id =
...
//                 error!(
//                     "Error while forwarding http connection {connection_id} (port {port}):
// {e:?}."                 )
//             } else {
//                 trace!(
//                     "Filtered http connection {connection_id} (port {port}) closed without
// errors."                 )
//             }
//         });
