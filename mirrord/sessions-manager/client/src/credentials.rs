...
/// them. Both callers are already inside a deadline-bounded async path, so a slow or failing
/// provider fails the attempt that needed it and is retried with it, rather than being handled
/// out of band.
