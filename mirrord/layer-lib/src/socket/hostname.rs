...
/// Returns Ok(None) on Bypassed remote hostname retrieval due to setup::local_hostname configuration
fn remote_hostname_string() -> HookResult<Option<String>> {
    // Check if hostname feature is enabled
    if setup().local_hostname() {
    ...
    trace!(
        "Successfully fetched hostname via proxy: {}",
        hostname
    );
    Ok(Some(hostname))
}
...
pub fn remote_dns_resolve_via_proxy(
    hostname: &str,
) -> HookResult<Vec<(String, std::net::IpAddr)>> {
