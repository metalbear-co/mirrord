...
///
/// Basename-aware and case-insensitive, ignoring a `.exe` suffix, so a full path
/// like `C:\...\bin\java.exe` or `/usr/bin/java` both match. This is what
/// [`DebuggerType::JavaAgent`] detection keys off: a plain `ends_with("java")`
/// misses the Windows `java.exe` that `mirrord pitm` spawns.
