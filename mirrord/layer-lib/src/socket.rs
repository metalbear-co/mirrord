    ...
    fn matches_with_resolver<R: DnsResolver>(
        ...
        let protocol_matches = matches!(
            ...
            (ProtocolFilter::Any, _)
                | (ProtocolFilter::Tcp, v2::OutgoingProtocol::Stream)
                | (ProtocolFilter::Udp, v2::OutgoingProtocol::Datagrams)
        );
        if !protocol_matches {
            return Ok(false);
        }
        ...
        let addr_protocol = if matches!(protocol, v2::OutgoingProtocol::Stream) {
            SOCK_STREAM
        } else {
            SOCK_DGRAM
        };
