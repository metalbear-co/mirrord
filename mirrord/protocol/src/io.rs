...
// Need these because the autogenerated impls add unnecessary bounds on T
impl<T> std::hash::Hash for QueueId<T> {
...
async fn io_task_chunked<IO: AsyncIO, Type: ProtocolEndpoint>(
    ...
    let mut out_queues = Arc::new(OutQueues {
