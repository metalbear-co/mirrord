...
use crate::{
    ...
    crd::{
        db_branching::{
            core::{
                BranchDatabasePhase, ConnectionParamsSpec,
...
/// Given parameters of all branch databases needed for a session, list reusable ones.
///
/// A branch is considered reusable if
...
pub async fn list_reusable_branches<P: Progress>(
    ...
    let reusable_branches = api
        ...
        .map(|db| (db.spec.common().id.to_string().into(), db))
        .collect::<HashMap<_, _>>();
        ...
        for branch_db_config in config.0.iter() {
            match branch_db_config {
                ...
                DatabaseBranchConfig::Pg(pg_config) => {
                    ...
                    let params = BranchParams::from_pg(
                        id.as_ref(),
                        pg_config,
                        target,
                        &session_target,
...
impl BranchParams {
    pub fn from_pg(
        ...
        let copy = match &config.copy {
            mirrord_config::feature::database_branches::PgBranchCopyConfig::Empty { tables } => {
                SqlBranchCopyConfig {
                    mode: SqlBranchCopyMode::Empty,
                    items: convert_copy_items(tables.clone()),
                }
            }
            mirrord_config::feature::database_branches::PgBranchCopyConfig::Schema { tables } => {
                SqlBranchCopyConfig {
                    mode: SqlBranchCopyMode::Schema,
                    items: convert_copy_items(tables.clone()),
                }
            }
            mirrord_config::feature::database_branches::PgBranchCopyConfig::All => {
                SqlBranchCopyConfig {
    ...
    pub fn from_mysql(
        ...
        let copy = match &config.copy {
            mirrord_config::feature::database_branches::MysqlBranchCopyConfig::Empty {
                tables,
            } => SqlBranchCopyConfig {
                mode: SqlBranchCopyMode::Empty,
                items: convert_copy_items(tables.clone()),
            },
            mirrord_config::feature::database_branches::MysqlBranchCopyConfig::Schema {
                tables,
            } => SqlBranchCopyConfig {
                mode: SqlBranchCopyMode::Schema,
                items: convert_copy_items(tables.clone()),
            },
            mirrord_config::feature::database_branches::MysqlBranchCopyConfig::All => {
                SqlBranchCopyConfig {
                    mode: SqlBranchCopyMode::All,
                    items: None,
                }
    ...
    pub fn from_mongodb(
        ...
        let copy = match &config.copy {
            mirrord_config::feature::database_branches::MongodbBranchCopyConfig::Empty {
                collections,
            } => MongodbBranchCopyConfig {
                mode: MongodbBranchCopyMode::Empty,
                items: convert_copy_items(collections.clone()),
            },
            mirrord_config::feature::database_branches::MongodbBranchCopyConfig::All {
                collections,
            } => MongodbBranchCopyConfig {
