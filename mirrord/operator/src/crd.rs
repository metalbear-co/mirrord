...
pub struct SqsQueueDetails {
    ...
    /// the target will be modified to include the environment variable `MEME_QUEUE_NAME`, with the name
    /// of the temporary queue as a value.
    /// Setting a fallback name only makes sense if the target application indeed uses the defined queue
    /// name source to override the source it uses on its absence.
