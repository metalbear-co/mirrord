...
/// Convenience trait for extracing [`ServerName`]s from an x509 certificate.
...
impl HasSubjectAlternateNames for CertificateDer<'_> {
    ...
    fn subject_alternate_names(&self) -> Result<Vec<ServerName<'static>>, GetSanError> {
        let certificate = X509Certificate::from_der(self)?.1;
        get_san(&certificate)
    }
}
...
impl HasSubjectAlternateNames for Pem {
    #[tracing::instrument(level = Level::DEBUG, skip(self), ret, err(level = Level::DEBUG))]
    fn subject_alternate_names(&self) -> Result<Vec<ServerName<'static>>, GetSanError> {
        let certificate = self.parse_x509()?;
        get_san(&certificate)
    }
}

fn get_san(certificate: &X509Certificate<'_>) -> Result<Vec<ServerName<'static>>, GetSanError> {
