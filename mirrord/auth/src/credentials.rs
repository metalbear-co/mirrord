...
impl bincode::Encode for CiApiKey {
    fn encode<E: enc::Encoder>(&self, encoder: &mut E) -> Result<(), EncodeError> {
        ...
        match self {
            CiApiKey::V1(credentials) => Encode::encode(&credentials, encoder),
        }
    }
}
...
pub mod client {
    ...
    impl CiApiKey {
        /// Prefix for v1 [`CiApiKey`].
        ///
        /// Keys with this prefix are generated by the operator with the `mirrord ci api-key`
        /// command, and they are validated against the operator license (if the operator doesn't
        /// support the `NewOperatorFeature::BypassCiCertificateVerification`), which means
        /// they break on license updates.
        ///
        /// For operator that support `BypassCiCertificateVerification`, this validation is
        /// bypassed.
        const V1_PREFIX: &str = "mci-v1:";

        /// Helper to get the correct prefix for this [`CiApiKey`].
        ///
        /// [`CiApiKey`] gets encoded with this prefix.
