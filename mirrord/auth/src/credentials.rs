...
impl Credentials {
    /// Create new instance of this struct.
    /// New instance is not ready (as in [`Credentials::is_ready`]) - it does not contain the
    /// certificate, which must be generated by the operator. Certification requests can be
    /// generated with [`Credentials::certificate_request`].
    ...
    /// Checks if [`Certificate`] exists in this struct and is valid in terms of expiration.
    ...
    /// The request is signed with the [`KeyPair`] stored in this struct.
...
pub mod client {
    ...
    impl Credentials {
        /// Create [`rfc2986::CertificationRequest`] and send it to the operator to replace
        /// [`Certificate`] stored in this struct, making this [`Credentials`] ready (as in
        /// [`Credentials::is_ready`]).
