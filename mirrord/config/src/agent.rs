...
pub struct AgentConfig {
    ...
    /// IP rather than to loopback. To avoid an iptables redirection loop, the agent excludes all
    /// of its own outgoing traffic from the redirect rules, so connections it makes to the
    /// pod's own IP for the outgoing feature are no longer redirected while this is enabled.
    ///
