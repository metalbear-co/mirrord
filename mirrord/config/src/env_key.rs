...
impl EnvKey {
    ...
    pub fn is_valid_kubernetes_label_value(value: &str) -> bool {
    ...
    pub fn to_hashed_label_value(value: &str) -> String {
        let hash = BASE64_URL_SAFE_NO_PAD.encode(Sha256::digest(value.as_bytes()));
...
mod tests {
    ...
    fn hashed_label_value_is_valid_for_valid_and_invalid_keys() {
        for key in [
            ...
            let label = EnvKey::to_hashed_label_value(key);

            assert!(
                EnvKey::is_valid_kubernetes_label_value(&label),
