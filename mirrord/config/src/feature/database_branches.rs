...
pub enum IamAuthConfig {
    ...
    ///   optionally `AWS_SESSION_TOKEN` from the target pod's environment (or from the env
    ///   vars named in the fields below) to the branch pod.
    /// - IRSA / EKS Pod Identity: when the target pod has no static keys, the branch pod
    ///   runs under the target's service account and receives the same IAM role. No key
    ///   fields are needed; `{ "type": "aws_rds" }` is enough.
    ///
    ...
    ///
    /// Parameters:
    /// - `region`: AWS region. If not specified, uses AWS_REGION or AWS_DEFAULT_REGION from
    ///   the target pod. With IRSA, set it explicitly if neither var is in the target's pod
    ///   spec.
