...
use crate::{
    ConnectionHandoffRequest, ConnectionHandoffResponse, ConnectionHandoffVerdict, error::Result,
};

/// Environment variable used by the layer and agent (workload-companion mode) to exchange accepted
...
pub async fn handle_connection_handoff_connection(
    ...
    let ReceivedConnectionHandoff {
        request,
        accepted_fd,
    } = receive_connection_handoff_with_fd(&unix_stream).await?;
...
async fn receive_connection_handoff_with_fd(
    stream: &UnixStream,
) -> Result<ReceivedConnectionHandoff> {
