...
use super::handoff::handoff_remote_connection;
use crate::claimed_sockets::{
    claim_placeholder_socket, claimed_socket, duplicate_claimed_socket, fill_address,
    remove_claimed_socket, socket_addr_from_fd, socket_peer_addr_from_fd,
};
...
#[hook_guard_fn]
pub(crate) unsafe extern "C" fn fcntl_detour(fd: c_int, cmd: c_int, arg: usize) -> c_int {
    unsafe {
        let result = FN_FCNTL(fd, cmd, arg);
        if result >= 0 && matches!(cmd, libc::F_DUPFD | libc::F_DUPFD_CLOEXEC) {
            duplicate_claimed_socket(fd, result);
        }
        result
    }
}
...
#[hook_guard_fn]
pub(crate) unsafe extern "C" fn dup_detour(fd: c_int) -> c_int {
    unsafe {
        let result = FN_DUP(fd);
        if result >= 0 {
            duplicate_claimed_socket(fd, result);
        }
        result
...
#[hook_guard_fn]
pub(crate) unsafe extern "C" fn dup2_detour(oldfd: c_int, newfd: c_int) -> c_int {
    unsafe {
        let result = FN_DUP2(oldfd, newfd);
        if result >= 0 {
            duplicate_claimed_socket(oldfd, result);
        }
        result
    }
}
...
pub(crate) unsafe extern "C" fn dup3_detour(oldfd: c_int, newfd: c_int, flags: c_int) -> c_int {
    unsafe {
        let result = FN_DUP3(oldfd, newfd, flags);
        if result >= 0 {
            duplicate_claimed_socket(oldfd, result);
        }
        result
