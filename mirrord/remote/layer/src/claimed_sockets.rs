...
pub(crate) fn connect_placeholder_socket(
    ...
    let fd_flags = fcntl(accepted_fd.as_raw_fd(), FcntlArg::F_GETFD)?;
    fcntl(
        fd.as_raw_fd(),
        FcntlArg::F_SETFD(FdFlag::from_bits_retain(fd_flags)),
    )?;

    let status_flags = fcntl(accepted_fd.as_raw_fd(), FcntlArg::F_GETFL)?;
    fcntl(
        fd.as_raw_fd(),
