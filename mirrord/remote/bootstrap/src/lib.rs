...
fn spawn_remote_flow() -> Result<()> {
    ...
    tracing::info!(agent_binary = %agent_binary.display(), ?connection_handoff_socket, "Launching workload-companion agent");
...
fn spawn_agent(binary: &Path) -> Result<()> {
    ...
    tracing::info!(pid = child.id(), "Spawned workload-companion agent");
