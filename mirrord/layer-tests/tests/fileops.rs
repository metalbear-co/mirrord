...
async fn read_from_mirrord_bin() {
    ...
    let (mut test_process, _intproxy) = application.start_process(vec![], None).await;
...
async fn go_dir(
    ...
    let (mut test_process, mut intproxy) = application
        .start_process(vec![("MIRRORD_FILE_READ_ONLY_PATTERN", "/tmp/foo")], None)
        .await;
...
async fn write_go(
    ...
    let (mut test_process, mut layer_connection) = Application::GoWrite(go_version)
        .start_process(get_rw_test_file_env_vars(), None)
        .await;
