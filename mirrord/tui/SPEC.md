...
| `PHASE` | `Pending` (amber), `Ready` (green), `Failed` (red), or `-` if the operator reported no status yet. |
...
Each env card's border is colored by its phase: green for `Ready`, yellow for `Idle`, purple for `Waiting`, dim purple for `Initializing`, red for `Failed`, gray for `Unknown`. The namespace/target blue and teal above are deliberately outside this set — MetalBear's brand palette has only three real hues (purple, yellow, mint) plus ink/grey neutrals, and phase already uses all of them, so distinguishing "namespace" and "target" from every phase color (and from each other) needs two colors the brand palette doesn't define.
