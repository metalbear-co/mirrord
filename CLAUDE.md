...
- **Operator**: Runs in-cluster for paid version (source in `../operator/`).
...
The `DetourGuard` pattern prevents infinite recursive during detour execution.
...
- **Imports at top of file**: Never use `use` statements inside functions. All imports must be at the beginning of the file.
- **Run `cargo fmt` after every change**: Always format code before committing.
- **Prefer `to_owned` for `&str` -> `String`**: Always use `to_owned` instead of `to_string` to turn a `&str` into a `String`.
- **Avoid superfluous inline comments**: Prefer function-level doc comments over "step-by-step" style inline comments that just repeat what the code says but in natural language.
