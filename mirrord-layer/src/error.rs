    ...
    fn from(fail: LayerError) -> Self {
        ...
        let libc_error = match fail {
            ...
            LayerError::ResponseError(response_fail) => match response_fail {
                ResponseError::AllocationFailure(_) => libc::EINVAL,
                ResponseError::NotFound(_) => libc::ENONET,
                ResponseError::NotDirectory(_) => libc::EINVAL,
                ResponseError::NotFile(_) => libc::EINVAL,
                ResponseError::RemoteIO(io_fail) => io_fail.raw_os_error.unwrap_or(libc::EIO),
            },
            LayerError::UnmatchedPong => libc::EINVAL,
