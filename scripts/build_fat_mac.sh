...
# The merged UI frontend is embedded via rust-embed from packages/ui/dist (build it first with
# `cargo xtask build-ui` for a working `mirrord ui`/`mirrord wizard`).
