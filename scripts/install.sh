...
#
# Usage:
#   curl -fsSL https://github.com/metalbear-co/mirrord/blob/latest/scripts/install.sh | sh
set -e
...
get_latest_version() {
  VERSION="$(curl -fsSL https://github.com/metalbear-co/mirrord/blob/latest/Cargo.toml | grep -m 1 version | cut -d' ' -f3 | tr -d '\"')"
  echo $VERSION
}
