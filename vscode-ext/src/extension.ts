...
function mirrordFailure(err: any) {
  vscode.window
    .showErrorMessage('mirrord failed to start. Please check the logs/errors.', 'Get help on Discord', 'Open an issue on GitHub', 'Send us an email')
    .then((value) => {
...
class MirrordAPI {
  ...
  constructor(context: vscode.ExtensionContext) {
    ...
    if (context.extensionMode === vscode.ExtensionMode.Development) {
      ...
      if (process.platform === 'darwin') {
        // macos binary is universal for all architectures
  ...
  async listTargets(configPath: string | null | undefined): Promise<string[]> {
    ...
    let lastTarget: string | undefined = globalContext.workspaceState.get(LAST_TARGET_KEY) || globalContext.globalState.get(LAST_TARGET_KEY);
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  buttons = {
    toggle: vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 0),
    settings: vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 0),
  };

  const toggleCommandId = 'mirrord.toggleMirroring';
  context.subscriptions.push(
    vscode.commands.registerCommand(toggleCommandId, async function () {
