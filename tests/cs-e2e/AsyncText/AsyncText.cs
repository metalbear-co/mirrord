...
// Vendored expected content. The agent serves the same bytes back; we
// don't need a runtime fetch.
const string ExpectedLoremIpsum =
...
// .NET on Windows accepts `/` as a separator and resolves `/app/test.txt`
// relative to the current drive, so this becomes the NT path
// `\??\<drive>:\app\test.txt` the layer sees and translates back to
// `/app/test.txt` for the agent.
