...
### Building mirrord layer

Run `cargo xtask build-layer` to build the mirrord layer library. 
This will result in a `target/<arch>/libmirrord_layer.dylib`.
Then, build the project with `MIRRORD_LAYER_FILE=$(pwd)/target/<arch>/debug/libmirrord_layer.dylib cargo build --all`
On macOS, always use `universal-apple-darwin` arch (rather than e.g. `aarch64-apple-darwin`) whenever a path to the layer binary is required 
...
Currently, **both integration and e2e tests** need you to pass the (absolute) path of both mirrord layer and mirrord binary as either command arguments...

```bash
cargo xtask test-e2e \
--binary $(pwd)/target/<arch>/debug/mirrord \
--layer $(pwd)/target/<arch>/debug/libmirrord_layer
```
