...
function parseEvent(event: SessionEvent): ParsedEvent {
  ...
  // Skip "at file:line" stack trace lines
  if (msg.trim().startsWith('at ') && msg.includes('/src/')) {
    return { type: event.type, summary: msg, skip: true }
  }

  // Config dumps: show friendly summary, keep raw data for dialog
  if (msg.includes('LayerConfig {') || msg.includes('incoming_mode=') || msg.includes('FeatureConfig')) {
    // Try to extract key config values for a nice summary
    const modeMatch = msg.match(/mode:\s*(Mirror|Steal)/i)
    const targetMatch = msg.match(/deployment:\s*"([^"]+)"/)
    const fsMatch = msg.match(/mode:\s*(Read|ReadWrite|Local|Disabled)/i)
    let summary = 'Session configuration loaded'
    const details: string[] = []
    if (targetMatch) details.push(`target: ${targetMatch[1]}`)
    if (modeMatch) details.push(`mode: ${modeMatch[1].toLowerCase()}`)
    if (fsMatch) details.push(`fs: ${fsMatch[1].toLowerCase()}`)
    if (details.length) summary += ` (${details.join(', ')})`

    // Pretty-print the raw config for the dialog
    const raw = msg
      .replace(/,\s*/g, ',\n')
      .replace(/\{/g, '{\n  ')
      .replace(/\}/g, '\n}')

    return { type: 'info', summary, rawData: raw, skip: false }
  }

  // Parse structured mirrord log lines
  const logMatch = msg.match(/^\d{4}-\d{2}-\d{2}T[\d:.]+Z\s+(INFO|WARN|ERROR|DEBUG|TRACE)\s+(\S+?):\s*(.+)$/s)
  if (logMatch) {
    const [, level, module, body] = logMatch
    const shortModule = module.split('::').pop() || module

    const type = level === 'WARN' ? 'warning' : level === 'ERROR' ? 'error' : 'info'

    // Friendly translations
    const launchMatch = body.match(/Launching "(.+?)" with arguments \[(.+?)\]/)
    if (launchMatch) {
      return { type: 'info', summary: `Launched process: ${launchMatch[1]} ${launchMatch[2].replace(/"/g, '')}`, rawData: msg, skip: false }
    }
    if (body.includes('Initializing mirrord-layer')) {
      return { type: 'info', summary: 'mirrord layer initialized', skip: false }
    }
    if (body.includes('Cancellation token triggered')) {
      return { type: 'info', summary: 'Proxy connection established', skip: false }
    }
    if (body.includes('incoming has changed')) {
      return { type: 'info', summary: 'Incoming traffic mode configured', rawData: msg, skip: false }
    }
    if (body.includes('picked first eligible container')) {
      return { type: 'warning', summary: 'Multiple containers found, picked first one', rawData: msg, skip: false }
    }

    return { type, summary: `${shortModule}: ${body.trim().substring(0, 200)}`, rawData: body.length > 200 ? msg : undefined, skip: false }
  }

  // ANSI color code cleanup
  const cleaned = msg.replace(/\x1b\[\d+m/g, '').trim()
  if (cleaned.length === 0) return { type: event.type, summary: msg, skip: true }

  // Parse ANSI-cleaned log lines
  const ansiLogMatch = cleaned.match(/^(\d{4}-\d{2}-\d{2}T[\d:.]+Z)\s+(INFO|WARN|ERROR|DEBUG|TRACE)\s+(\S+?)\s*(.+)$/s)
  if (ansiLogMatch) {
    const [, , level, module, body] = ansiLogMatch
    const shortModule = module.split('::').pop() || module
    const type = level === 'WARN' ? 'warning' : level === 'ERROR' ? 'error' : 'info'
    ...
    if (body.includes('incoming has changed')) {
      return { type: 'info', summary: 'Incoming traffic mode configured', rawData: cleaned, skip: false }
    }

    return { type, summary: `${shortModule}: ${body.trim().substring(0, 200)}`, rawData: body.length > 200 ? cleaned : undefined, skip: false }
  }

  // Our own events (no source field = generated by us)
  if (!d.source) {
    return { type: event.type, summary: msg, skip: false }
  }

  if (cleaned) {
    return { type: event.type, summary: cleaned.substring(0, 200), rawData: cleaned.length > 200 ? cleaned : undefined, skip: false }
  }
