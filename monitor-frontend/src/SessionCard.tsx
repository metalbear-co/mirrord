...
import { Cpu, Clock, Server, Trash2, Zap } from 'lucide-react'
    ...
    >
      <CardContent className="p-3 space-y-2">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          {session.mode && (
          <Badge
            variant={session.mode === 'steal' ? 'destructive' : 'secondary'}
            className="shrink-0 uppercase text-[10px] tracking-wider"
          >
            {session.mode === 'steal' && <Zap className="h-3 w-3 mr-0.5" />}
            {session.mode}
          </Badge>
          )}
        </div>
        ...
        <div className="flex items-center gap-1.5">
          ...
          {(session.ports || []).map((port) => (
            <Badge
              key={port}
              variant="outline"
              className="font-mono text-[10px] px-1.5 py-0 text-primary-light"
            >
              {port}
            </Badge>
          ))}
